...
// Shared content for the two MCP nav sections (see app/dashboard/servers/):
//   - mode "mine":    /dashboard/servers         -> your active connections
//                    (unified proxy URL, live connection test, disconnect)
//   - mode "library": /dashboard/servers/library -> the browsable, searchable
//                    catalog of admin-approved templates. "Connect" opens a
//                    schema-driven wizard that stores credentials (encrypted
//                    at rest). Superusers additionally get the Tool Discovery
//                    panel (captures real tools/list schemas per template).
//
...
export default function MCPServersPanel({ mode }: Props) {
  ...
  // The discovery state (superuser) is only shown in the library section -
  // load it there on mount so the panel is current without a prior run.
  useEffect(() => {
    if (mode === 'library') loadDiscovery();
  }, [mode, loadDiscovery]);
  ...
  const title = mode === 'library' ? 'MCP Library' : 'My MCP Servers';
  const subtitle =
    mode === 'library'
      ? 'Admin-approved integrations. Credentials encrypted at rest.'
      : 'Your active connections. Credentials encrypted at rest.';

  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      <header className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-2">
        <div>
          <h2 className="font-pixel font-bold text-2xl sm:text-3xl text-ink text-px-sm">
            {title}
            {mode === 'library' && !loading && (
              <span className="text-base font-normal text-ink-dim"> ({templates.length})</span>
            )}
          </h2>
              ...
              <div className="flex justify-end mb-4">
                <div className="relative w-full sm:w-72">
                  ...
                  <input
                    ...
                    placeholder={`Search ${templates.length} integrations...`}
        ...
        <section className="panel pixel-caps p-4 sm:p-6 md:p-8 [--cap:theme('colors.eepy.sage')]">
          {configs.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center space-y-2 border-2 border-dashed border-night-border">
              ...
              <Link
                href="/dashboard/servers/library"
